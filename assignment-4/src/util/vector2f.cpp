...
float &Vector2f::operator[](int index) { return v[index]; }

float Vector2f::operator[](int index) const { return v[index]; }
...
Vector2f Vector2f::operator-() const { return Vector2f(-v[0], -v[1]); }
...
float Vector2f::magnitude() const { return sqrt(v[0] * v[0] + v[1] * v[1]); }

float Vector2f::magnitudeSquared() const { return v[0] * v[0] + v[1] * v[1]; }
...
Vector2f operator*(float scale, const Vector2f &v) { return v * scale; }
